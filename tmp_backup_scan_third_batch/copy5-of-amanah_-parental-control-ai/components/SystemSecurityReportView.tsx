  ...
  if (isAuthorized === false) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-10 space-y-6">
         <div className="w-32 h-32 bg-red-50 text-red-600 rounded-[3rem] flex items-center justify-center text-6xl shadow-inner animate-pulse">🚫</div>
         <h2 className="text-3xl font-black text-slate-900">غير مصرح لك بالدخول</h2>
         <p className="text-slate-500 font-bold max-w-md">تقارير النزاهة وأدوات الحقن السحابية قاصرة فقط على فرق العمليات السيادية (Developers/SOC/SRE). تم تسجيل محاولة الدخول هذه.</p>
      </div>
    );
  }

  if (isAuthorized === null) return <div className="p-20 text-center font-black animate-pulse text-indigo-600">Verifying Security Credentials...</div>;
