...
const DashboardView: React.FC<DashboardViewProps> = ({ children, alerts, onTriggerDemo, lang, parentId }) => {
  ...
  return (
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      {/* 🔑 بطاقة الربط السريع - تظهر بشكل بارز إذا لم تكن هناك أجهزة */}
      {children.length === 0 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="space-y-3">
              {alerts.length > 0 ? alerts.slice(0, 4).map(alert => (
                ...
                <div className="text-center py-24 flex flex-col items-center gap-6 opacity-30">
                  ...
                  <p className="font-bold text-slate-500 italic">لا توجد تنبيهات حقيقية. قم بربط الجهاز للبدء.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-10">
          <div className="bg-slate-900 rounded-[3.5rem] p-10 shadow-2xl text-white relative overflow-hidden group min-h-[500px]">
            ...
            {children.length > 0 ? children.map(child => (
              <div key={child.id} className="space-y-6 relative z-10 mb-10 last:mb-0">
                <div className="flex items-center gap-6">
                  <img src={child.avatar} className="w-16 h-16 rounded-full object-cover shadow-lg border-2 border-white/10" />
                  <div>
                    <p className="font-black text-xl tracking-tight leading-none mb-1">{child.name}</p>
                    <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">العمر: {child.age} {lang === 'ar' ? 'سنة' : 'Years'}</p>
                  </div>
                </div>
                {child.psychProfile && (
              ...
              <div className="text-center py-20 flex flex-col items-center gap-6 opacity-40">
                 <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-4xl border border-white/10">🧠</div>
                 <p className="font-bold text-xs text-indigo-200">بانتظار ربط هاتف طفلك لتحليل النبض النفسي.</p>
              </div>
            )}
          </div>
        </div>
