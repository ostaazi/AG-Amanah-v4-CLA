...
const AuthView: React.FC<AuthViewProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 relative overflow-hidden" dir="rtl">
      {/* تضمين التعريفات البصرية لضمان عمل اللوجو والدرع */}
      <AmanahGlobalDefs />
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-3xl border border-white/10 p-10 rounded-[4rem] shadow-2xl relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-[#8A1538] font-bold transition-all focus:ring-4 focus:ring-[#8A1538]/10"
            />
          </div>
          ...
          {error && <div className="p-4 bg-red-500/20 border border-red-500/50 rounded-2xl text-red-200 text-[10px] font-black text-center animate-shake">⚠️ {error}</div>}
          {successMsg && <div className="p-4 bg-emerald-500/20 border border-emerald-500/50 rounded-2xl text-emerald-200 text-[10px] font-black text-center animate-in zoom-in">✅ {successMsg}</div>}
          ...
          >
            {loading ? 'جاري التحميل...' : (mode === 'LOGIN' ? 'دخول النظام' : mode === 'REGISTER' ? 'إنشاء حساب أمان' : 'إرسال الرابط')}
          </button>
        </form>
        ...
        <div className="mt-8 text-center">
             ...
             >
               {mode === 'LOGIN' ? 'ليس لديك حساب؟ انضم لعائلة أمان' : 'لديك حساب بالفعل؟ سجل دخولك'}
             </button>
        </div>
      </div>
    </div>
