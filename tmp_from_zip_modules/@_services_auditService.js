import { verifyEncryptionIntegrity } from '@/services/cryptoService';
import { applySystemPatchCloud, rollbackSystemPatchCloud } from '@/services/firestoreService';
// الحالة المحلية لمزامنة الواجهة فقط
let currentPatchedIds = [];
export const setAuditCache = (ids) => {
    currentPatchedIds = ids;
};
export const applySystemPatch = async (parentId, id) => {
    console.log(`[Amanah Cloud Audit] Pushing Patch ${id} to Sovereign Database...`);
    await applySystemPatchCloud(parentId, id);
};
export const rollbackSystemPatch = async (parentId, id) => {
