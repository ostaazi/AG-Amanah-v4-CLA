 ...
 */
export const generateEncryptedBackup = async (parentId) => {
    if (!db)
        return null;
    try {
        ...
        for (const colName of collectionsToBackup) {
            ...
            backupData.payload[colName] = snap.docs
                ...
                .filter((d) => d.parentId === parentId || d.id === parentId);
        }
...
export const restoreFromEncryptedBackup = async (encryptedData, parentId) => {
    if (!db)
        return false;
    try {
        ...
        if (backup.parentId !== parentId)
            throw new Error("Security Violation: Parent ID mismatch");
