...
import { CommandType, ProtocolStatus, CommandStatus, } from '@prisma/client';
...
function actionToCommand(action, ctx) {
    switch (action) {
        case 'APP_KILL':
...
export async function enqueueCommands(params) {
    ...
    if (commands.length === 0)
        return [];
    const created = await prisma.$transaction(commands.map((c) => prisma.deviceCommand.create({
        data: {
            family_id,
            device_id,
