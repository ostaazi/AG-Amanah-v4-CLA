 ...
 */
export const purifyToSkeleton = (input) => {
    if (!input)
        return "";
 ...
 */
export const localSentinelCheck = (text) => {
    // Fix: Added `latency: "0ms"` to ensure return type consistency with other return statements
    if (!text)
        return { isDanger: false, category: Category.SAFE, severity: AlertSeverity.LOW, skeleton: "", latency: "0ms" };
