...
export async function POST(req) {
    try {
        ...
        const incident_type = cleanEnum(body?.incident_type);
        const severity = cleanEnum(body?.severity);
        const content_type = cleanEnum(body?.content_type);
        ...
        const tags = Array.isArray(body?.tags) ? body.tags.slice(0, 10).map((x) => String(x).trim().toLowerCase()) : [];
        const app_package = body?.app_package ? String(body.app_package).slice(0, 120) : null;
        if (!summary || summary.length < 10)
            return jsonError(400, 'summary required (min 10 chars)');
        if (!sha256 || sha256.length < 16)
            return jsonError(400, 'sha256 required');
