          ...
          <div className="flex-1 relative bg-slate-100 flex items-center justify-center overflow-hidden">
            {child.location ? (
              <iframe
                ...
                frameBorder="0"
                scrolling="no"
                src={`https://maps.google.com/maps?q=${child.location.lat},${child.location.lng}&z=15&output=embed`}
              ...
              <div className="flex flex-col items-center gap-6 opacity-30">
                ...
                <p className="font-black text-xl text-center">
                  بانتظار إشارة الـ GPS لجهاز {child.name}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900 rounded-[3rem] p-8 shadow-2xl text-white relative overflow-hidden h-full flex flex-col justify-between group">
            <div className="space-y-6 relative z-10">
              ...
              <div className="p-6 bg-white/5 border border-white/10 rounded-2xl italic text-xs font-bold leading-relaxed text-indigo-100">
                {locationIntel?.text ||
                  'بانتظار بيانات الموقع لتحليل جودة المنطقة المحيطة ودرجة الأمان.'}
              </div>
            </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-6">
          {childApps.map((app) => (
            <button
              ...
              className={`relative w-full aspect-square rounded-[2.5rem] flex flex-col items-center justify-center transition-all border-4 ${app.isBlocked ? 'bg-red-50 border-red-500 shadow-lg' : 'bg-white border-slate-50 hover:scale-105 shadow-sm'}`}
            >
