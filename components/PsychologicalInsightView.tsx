...
const guidanceScenariosBase: GuidanceScenario[] = [
  ...
  },
  {
    ...
    prevention: [
      ...
      'تفعيل المصادقة الثنائية (2FA) للحسابات الرئيسية.',
    ...
    incidentPlan: [
      ...
      'تأمين الحسابات (تغيير كلمة المرور + المصادقة الثنائية (2FA)).',
  ...
  },
  {
    ...
    prevention: [
      ...
      'تفعيل المصادقة الثنائية (2FA) وعدم مشاركة رموز التحقق مطلقاً.',
    ...
    interventionProgram: [
      ...
      { week: 'الأسبوع 2', goal: 'تحصين الحسابات', action: 'تفعيل المصادقة الثنائية (2FA) لكل الحسابات ومراجعة إعدادات الاسترداد.' },
    ...
    incidentPlan: [
      ...
      'تفعيل المصادقة الثنائية (2FA) وإغلاق كل الجلسات النشطة غير الموثوقة.',
  ...
  },
  {
    ...
    interventionProgram: [
      ...
      { week: 'الأسبوع 2', goal: 'أمان رقمي', action: 'تغيير كلمات المرور وتفعيل المصادقة الثنائية (2FA) ومراجعة الجلسات.' },
  ...
  },
  {
    ...
    prevention: [
      'تفعيل المصادقة الثنائية (2FA) على كل الحسابات المهمة.',
...
const scenarioKnowledgeExtensions: Record<PsychScenarioId, GuidanceScenarioExtension> = {
  ...
  account_theft_fraud: {
    ...
    prevention: [
      ...
      'تفعيل المصادقة الثنائية (2FA) وربط الحساب ببريد استرداد آمن ومستقل.',
...
const PsychologicalInsightView: React.FC<PsychologicalInsightViewProps> = ({
  ...
  const fallbackProfile = useMemo<PsychologicalProfile>(
    () => ({
      anxietyLevel: 35,
      moodScore: 55,
      dominantEmotion: lang === 'ar' ? 'مستقر' : 'Stable',
      isolationRisk: 20,
      recentKeywords: [],
      recommendation:
        lang === 'ar'
          ? 'جاري تحميل ملف النبض النفسي. يرجى الانتظار...'
  ...
  const radarData = [
    { subject: 'قلق', A: anxietyScore, fullMark: 100 },
    { subject: 'إحباط', A: frustrationScore, fullMark: 100 },
    { subject: 'عزلة', A: isolationScore, fullMark: 100 },
    { subject: 'عدوانية', A: aggressionScore, fullMark: 100 },
    { subject: 'هدوء', A: calmScore, fullMark: 100 },
  ];
  ...
  const weeklyTrend =
    profile.weeklyTrend && profile.weeklyTrend.length > 0
      ? profile.weeklyTrend
      : [
          { label: 'الاثنين', value: clampScore(trendSeed - 6) },
          { label: 'الثلاثاء', value: clampScore(trendSeed - 2) },
          { label: 'الأربعاء', value: clampScore(trendSeed + 1) },
          { label: 'الخميس', value: clampScore(trendSeed + 4) },
          { label: 'الجمعة', value: clampScore(trendSeed + 7) },
        ];
  ...
  const runAutoExecutionPlan = async () => {
    ...
    if (resolvedTargetChildIds.length === 0) {
      ...
      setAutoRunSummary(
        lang === 'ar'
          ? 'لا يمكن تنفيذ الخطة: لا توجد أجهزة مرتبطة بهذا الطفل.'
          : 'Cannot execute plan: no linked devices found for this child.'
      );
      pushTimeline({
        title: lang === 'ar' ? 'تعذر التنفيذ' : 'Execution unavailable',
        detail:
          lang === 'ar'
            ? 'لا توجد أجهزة مرتبطة بهذا الطفل لتنفيذ الإجراءات عليها.'
      ...
      try {
        ...
        if (failedTargets === 0) {
          ...
          pushTimeline({
            ...
            detail:
              lang === 'ar'
                ? `تم التنفيذ بنجاح على ${resolvedTargetChildIds.length} جهاز.`
          ...
          pushTimeline({
            ...
            detail:
              lang === 'ar'
                ? `تم التنفيذ جزئيًا (${succeededTargets} ناجح / ${failedTargets} فشل).`
      ...
      try {
        ...
        if (failedTargets === 0) {
          pushTimeline({
            ...
            detail:
              lang === 'ar'
                ? `تم تطبيق الوضع على ${resolvedTargetChildIds.length} جهاز للطفل ${child.name}.`
          ...
          pushTimeline({
            ...
            detail:
              lang === 'ar'
                ? `تم التطبيق جزئيًا (${succeededTargets} ناجح / ${failedTargets} فشل).`
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-72 animate-in fade-in" dir="rtl">
      <div className="bg-white rounded-[2.5rem] p-5 border border-slate-100 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 text-center text-[10px] font-black tracking-wide text-slate-400 uppercase">
            {lang === 'ar' ? 'اختر الطفل' : 'Select Child'}
          </div>
          <button
            type="button"
              ...
              >
                ...
                <div className="text-right">
                  ...
                  >
                    {group.deviceCount > 1
                      ? lang === 'ar'
                        ? `${group.deviceCount} أجهزة مرتبطة`
                        : `${group.deviceCount} linked devices`
                      : item.deviceNickname || (lang === 'ar' ? 'جهاز الطفل' : 'Child Device')}
                  </span>
                </div>
              </button>
            );
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white border border-indigo-100 rounded-2xl p-4 text-center">
                <p className="text-xs font-black text-slate-500 mb-1">جاهزية إدارة الحادث</p>
                <p className="text-3xl font-black text-indigo-700">{readinessScore}</p>
              </div>
              <div className="bg-white border border-indigo-100 rounded-2xl p-4 text-center">
                <p className="text-xs font-black text-slate-500 mb-1">العاطفة الغالبة</p>
      ...
      <section className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-xl space-y-6">
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-5">
          <p className="text-xs font-black text-slate-500 mb-3">اتجاه النبض خلال الأسبوع</p>
      ...
      <section className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-xl space-y-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h3 className="text-xl font-black text-slate-900">
              {lang === 'ar' ? 'التوقع الاستباقي للمخاطر' : 'Proactive Risk Forecast'}
            </h3>
            <p className="text-xs font-bold text-slate-500 mt-1">
              {lang === 'ar'
                ? 'تحليل سياق الحوار وتكرار الأنماط لتوقع ما قد يواجهه الطفل خلال 7 و30 يوم.'
        ...
        {sevenDayTopForecast || thirtyDayTopForecast ? (
          ...
          <p className="text-xs font-bold text-slate-500">
            {lang === 'ar'
              ? 'لا توجد بيانات كافية للتوقع حتى الآن. سيظهر التوقع بعد تراكم سياق محادثات كاف.'
        ...
        {psychForecast.context.repeatedTerms.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-2">
            <p className="text-xs font-black text-slate-500">
              {lang === 'ar' ? 'أكثر الكلمات تكرارًا في السياق' : 'Most repeated context terms'}
            </p>
        ...
        {psychForecast.signalFusion.trajectories.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-3">
            <p className="text-xs font-black text-slate-500">
              {lang === 'ar' ? 'المسارات التنبؤية متعددة المصادر' : 'Multi-source predictive trajectories'}
            </p>
        ...
        {psychForecast.signalFusion.topDriversEn.length > 0 && (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-2">
            <p className="text-xs font-black text-slate-500">
              {lang === 'ar' ? 'أقوى دوافع التوقع متعددة المصادر' : 'Top multi-source forecast drivers'}
            </p>
            <div className="flex flex-wrap gap-2">
                  ...
                  {!isExpanded && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-3 space-y-2 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-black text-slate-500">الأعراض (أعلى إشارتين)</p>
                          <button
                            type="button"
                          ...
                          {scenario.symptoms.slice(0, 2).map((item, idx) => {
                            ...
                            return (
                              <article key={`${item}-${idx}`} className="rounded-2xl border border-slate-100 bg-white/95 p-3 space-y-1.5 shadow-sm">
                                ...
                                <p className="text-sm font-bold text-slate-600">لماذا الآن: {preview.whyNow}</p>
                      ...
                      <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-3 space-y-2 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-black text-slate-500">طرق الاستدراج (أعلى نمطين)</p>
                          <button
                            type="button"
                  ...
                  {isExpanded && (
                    <div className="space-y-4 pt-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-black text-slate-500">الأعراض التفصيلية</p>
                            <button
                              type="button"
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-black text-slate-500">طرق الاستدراج التفصيلية</p>
                            <button
                              type="button"
                      ...
                      <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-4 space-y-2 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-black text-slate-500">إجراءات الوقاية (موسعة)</p>
                          <button
                            type="button"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-4 space-y-2 shadow-sm">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-black text-slate-500">خطة 10 دقائق (تفصيل)</p>
                            <button
                              type="button"
                        ...
                        <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-4 space-y-2 shadow-sm">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-black text-slate-500">رسائل التنبيه (تفصيل)</p>
                            <button
                              type="button"
                      ...
                      <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-4 space-y-2 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-black text-slate-500">برنامج التدخل العلاجي (موسع)</p>
                          <button
                            type="button"
                      ...
                      <div className="rounded-2xl bg-slate-50/85 border border-slate-100 p-4 space-y-2 shadow-sm">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-sm font-black text-slate-500">الحوارات التربوية (موسعة)</p>
                          <button
                            type="button"
