...
const EvidenceVaultView: React.FC<EvidenceVaultViewProps> = ({ records, currentUser, onModalToggle, onRequestToast, isLoading = false }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-40 animate-in fade-in no-print" dir="rtl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-4 px-2 animate-in">
        {isLoading ? (
          [1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="bg-white rounded-[3rem] border border-slate-100 shadow-lg p-8 space-y-6">
          ...
          filteredRecords.map((record) => (
            <div key={record.id} onClick={() => setSelectedRecord(record)} className={`bg-white rounded-[3rem] border shadow-lg p-8 cursor-pointer hover:border-indigo-600 transition-all duration-300 hover:scale-[1.02] group ${selectedRecord?.id === record.id ? 'border-indigo-600 ring-4 ring-indigo-50 shadow-2xl' : 'border-slate-100'}`}>
