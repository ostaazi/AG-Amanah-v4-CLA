    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-[#8A1538] rounded-[3rem] p-10 text-white shadow-2xl border-b-8 border-amber-500/50 flex flex-col md:flex-row items-center justify-between gap-8 animate-bounce-subtle">
          <div className="text-right flex-1">
            ...
            <p className="text-slate-300 font-bold text-sm">
              أدخل هذا الرمز في تطبيق طفلك لتبدأ الرقابة الحقيقية فوراً.
            </p>
          </div>
      ...
      <div className="bg-slate-900 rounded-[3.5rem] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl border-b-4 border-[#8A1538]">
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-10 text-center lg:text-right">
          <div>
            ...
            <p className="text-indigo-200 text-lg font-bold opacity-80 max-w-xl">
              {t.monitoringMsg}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in">
        <div className="lg:col-span-8 space-y-10">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <QuickStat
                label={t.screenHours}
                value={children.length > 0 ? '4.2' : '0'}
                sub={t.hour}
                color="indigo"
              />
              <QuickStat
                label={t.blockedSites}
                value={children.length > 0 ? '12' : '0'}
                color="amber"
              />
              <QuickStat
                label={t.linkStatus}
                value={children.length > 0 ? t.secure : 'غير مربوط'}
                color="green"
              />
            </div>
          )}
          ...
          <div className="bg-white/70 backdrop-blur-2xl p-8 rounded-[4rem] border border-white shadow-xl min-h-[400px] slide-up stagger-1">
              ...
              {isActivitiesLoading || isLoading ? (
                ...
                alerts.slice(0, 5).map((alert) => (
                  ...
                  >
                    ...
                    <div className="text-left flex flex-col items-end">
                      <div
                        className={`px-4 py-1.5 rounded-full text-[9px] font-black mb-1 ${alert.severity === AlertSeverity.CRITICAL ? 'bg-red-600 text-white' : 'bg-slate-200 text-slate-600'}`}
                      >
                        {alert.severity || 'LOW'}
                      </div>
                ...
                <div className="text-center py-24 flex flex-col items-center gap-6 opacity-40">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="font-bold text-slate-400 text-sm max-w-xs mx-auto">
                      تأكد من ربط تطبيق الطفل وتفعيل صلاحيات "Accessibility" للبدء بالرصد.
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="lg:col-span-4 space-y-10">
          <div className="bg-slate-900 rounded-[3.5rem] p-10 shadow-2xl text-white relative overflow-hidden group min-h-[500px] border-b-8 border-[#D1A23D] slide-up stagger-2">
            <h3 className="text-2xl font-black mb-8 relative z-10 tracking-tighter">
              {t.psychPulse}
            </h3>
            {isLoading ? (
              <div className="space-y-6 relative z-10 mb-10">
                <div className="flex items-center gap-6">
                  <Skeleton variant="circle" className="w-16 h-16 bg-white/10" />
                  <div className="space-y-2">
                    <Skeleton variant="text" className="w-32 h-6 bg-white/10" />
                    <Skeleton variant="text" className="w-20 h-3 bg-white/5" />
                  </div>
                </div>
                ...
                <div key={child.id} className="space-y-6 relative z-10 mb-10 last:mb-0">
                  <div className="flex items-center gap-6">
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 border-slate-900 ${child.status === 'online' ? 'bg-emerald-500' : 'bg-slate-500'}`}
                      ></div>
                    </div>
                    <div>
                    ...
                    <div className="bg-white/5 p-6 rounded-[2.5rem] border border-white/10">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-xs font-bold">
                          {child.psychProfile.dominantEmotion}
                        </span>
                      </div>
                      <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                        <div
                          className={`h-full bg-indigo-500`}
                          style={{ width: `${child.psychProfile.anxietyLevel}%` }}
                        ></div>
                      </div>
                    </div>
                  )}
                  <button
                    ...
                    className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 rounded-2xl text-sm font-black transition-all shadow-xl"
                  >
                    {t.detailedReport}
                  </button>
                </div>
              ...
              <div className="text-center py-20 flex flex-col items-center gap-6 opacity-40">
                <div className="w-24 h-24 bg-white/5 rounded-3xl flex items-center justify-center text-5xl border border-white/10 shadow-inner">
                  🔒
                </div>
                <p className="font-bold text-xs text-indigo-200 leading-relaxed">
                  بانتظار ربط هاتف طفلك لتحليل النبض النفسي وحالة الاستقرار.
                </p>
              </div>
            )}
          </div>
...
const QuickStat: React.FC<{ label: string; value: any; sub?: string; color: string }> = ({
  label,
  value,
  sub,
  color,
  ...
  return (
    <div
      className={`p-6 rounded-[2.5rem] border ${colors[color]} shadow-sm transition-all hover:shadow-md hover:scale-[1.02]`}
    >
      <p className="text-[8px] font-black uppercase tracking-widest mb-1 opacity-70">{label}</p>
      <p className="text-2xl font-black tracking-tighter">
        {value} <span className="text-[10px]">{sub}</span>
      </p>
    </div>
  );
};
