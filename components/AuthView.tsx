      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-3xl border border-white/10 p-10 rounded-[4rem] shadow-2xl relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase px-4 tracking-widest">
              البريد الإلكتروني
            </label>
            <input
              type="email"
          ...
          {error && (
            <div className="p-4 bg-red-500/20 border border-red-500/50 rounded-2xl text-red-200 text-[10px] font-black text-center animate-shake">
              ⚠️ {error}
            </div>
          )}
          {successMsg && (
            <div className="p-4 bg-emerald-500/20 border border-emerald-500/50 rounded-2xl text-emerald-200 text-[10px] font-black text-center animate-in zoom-in">
              ✅ {successMsg}
            </div>
          )}
