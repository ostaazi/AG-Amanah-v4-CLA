...
const NotificationToast: React.FC<NotificationToastProps> = ({ alert, onClose }) => {
  ...
  const qatarGold =
    'linear-gradient(135deg, #FFF8D8 0%, #F7DE8D 10%, #D1A23D 22%, #FFF2B6 36%, #B47E1B 50%, #FFE6A0 64%, #C69126 78%, #7A4D0A 100%)';

  return (
    <div
      className={`fixed top-12 left-0 right-0 z-[9999] px-6 transition-all duration-700 ease-out transform ${visible ? 'translate-y-0 opacity-100' : '-translate-y-32 opacity-0'}`}
    >
      <div
      ...
      >
        ...
        <div className="flex-1 min-w-0 text-right relative z-10">
          <div className="flex justify-between items-center flex-row-reverse mb-0.5">
            <p className="text-[9px] font-black uppercase tracking-widest text-[#8A1538]/50">
              Amanah Security • الآن
            </p>
            <div
              className={`w-1.5 h-1.5 rounded-full ${isSystemSuccess ? 'bg-[#D1A23D]' : 'bg-red-600'} animate-pulse`}
            ></div>
          </div>
