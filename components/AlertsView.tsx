  ...
  const filteredAlerts = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        (a) => a.content?.toLowerCase().includes(term) || a.category.toLowerCase().includes(term)
      );
    }
    return result;
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2">
            {lang === 'ar'
              ? 'اضغط على أي تنبيه لفتح ملف الأدلة الجنائية الخاص به.'
              : 'Click any alert to open its forensic evidence file.'}
          </p>
        </div>
      </div>
              ...
              <div className="flex-1 space-y-6">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <span
                      className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${alert.severity === AlertSeverity.CRITICAL ? 'bg-red-600 text-white' : 'bg-amber-400 text-white'}`}
                    >
                      {alert.severity}
                    </span>
                    <span
                      className={`text-[10px] font-black px-4 py-2 rounded-full ${isDark ? 'bg-indigo-900/40 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}
                    >
                      {alert.platform}
                    </span>
                  </div>
                ...
                <div>
                  ...
                  >
                    {alert.childName} <span className="text-slate-300 mx-2">•</span>{' '}
                    {alert.category}
                  </h3>
                  <div
                    className={`${isDark ? 'bg-slate-800' : 'bg-slate-50'} p-6 rounded-[2rem] border ${isDark ? 'border-white/5' : 'border-slate-100'} mt-4 italic font-bold text-slate-700`}
                  >
                    "{alert.content}"
                  </div>
                </div>
                ...
                <div
                  className={`${isDark ? 'bg-indigo-950/20' : 'bg-indigo-50/50'} p-8 rounded-[2.5rem] border border-indigo-100 relative overflow-hidden`}
                >
                  <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h4
                        className={`text-[11px] font-black uppercase tracking-widest ${isDark ? 'text-indigo-300' : 'text-slate-800'}`}
                      >
                        Amanah AI Analysis
                      </h4>
                    </div>
                    <p
                      className={`text-sm font-bold leading-relaxed mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
                    >
                      {alert.aiAnalysis}
                    </p>
                    <div
                      className={`flex items-center justify-between p-4 rounded-2xl shadow-sm border ${isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-indigo-100'}`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        >
                          ...
                          <span className="text-red-600 font-black">{alert.actionTaken}</span>
                        </span>
                      </div>
                    </div>
                  </div>
