...
const DashboardView: React.FC<DashboardViewProps> = ({ children, alerts, onTriggerDemo }) => {
    ...
    <div className="space-y-10 pb-10 animate-in fade-in duration-700">
      ...
      <div className="bg-slate-900 rounded-[3.5rem] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl border-b-4 border-indigo-600">
        <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-10 text-center lg:text-right">
          <div>
            <h2 className="text-4xl md:text-5xl font-black mb-4 tracking-tighter">نظام الحماية نشط 🛡️</h2>
            <p className="text-indigo-200 text-lg font-bold opacity-80 max-w-xl">مرحباً محمد، نظام Amanah يراقب الآن 42 منصة تواصل اجتماعي لضمان سلامة أطفالك.</p>
          </div>
          <button 
          ...
          >
            ...
            <span className="text-sm font-black tracking-tighter uppercase">اختبار اختراق تجريبي</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <QuickStat label="تنبيهات اليوم" value={alerts.length} color="red" />
            <QuickStat label="ساعات الشاشة" value="4.2" sub="ساعة" color="indigo" />
            <QuickStat label="مواقع محظورة" value="12" color="amber" />
            <QuickStat label="حالة الربط" value="مؤمن" color="green" />
          </div>

          {/* Recent Alerts Card */}
          <div className="bg-white/70 backdrop-blur-2xl p-8 rounded-[4rem] border border-white shadow-xl">
            <div className="flex justify-between items-center mb-8 px-4">
              <h3 className="text-2xl font-black text-slate-800 tracking-tighter">آخر الأنشطة المرصودة</h3>
              <Link to="/alerts" className="text-indigo-600 font-black text-xs uppercase tracking-widest hover:underline">السجل الكامل</Link>
            </div>
            ...
            <div className="space-y-3">
              {recentAlerts.length > 0 ? recentAlerts.map(alert => (
                ...
                <div className="text-center py-10 text-slate-400 font-bold italic">لا يوجد تنبيهات حالياً</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white/70 backdrop-blur-2xl p-8 rounded-[4rem] border border-white shadow-xl">
            <h3 className="text-2xl font-black text-slate-800 mb-8 px-4 tracking-tighter">توزيع وقت الشاشة</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div className="h-64 relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">مراقب</span>
                </div>
              </div>
              <div className="space-y-4">
                {appUsageData.map((app, idx) => (
                  <div key={app.name} className="flex justify-between items-center">
                    ...
                    <span className="text-xs font-bold text-slate-400">{app.value} دقيقة</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          <div className="bg-slate-900 rounded-[3.5rem] p-10 shadow-2xl text-white relative overflow-hidden group">
            <h3 className="text-2xl font-black mb-8 relative z-10 tracking-tighter">النبض النفسي الآلي</h3>
            {children.map(child => (
              <div key={child.id} className="space-y-6 relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">آخر تحليل: اليوم 10:30 ص</p>
                  </div>
                </div>
                {child.psychProfile && (
                  <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-[10px] font-black text-indigo-300 uppercase">مؤشر القلق</span>
                ...
                <button onClick={() => navigate('/pulse')} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 rounded-2xl text-xs font-black transition-all shadow-xl">عرض التقرير العاطفي المفصل</button>
              </div>
            ))}
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-4">
            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest px-2">إجراءات سريعة</h4>
            <div className="grid grid-cols-2 gap-3">
              <QuickAction icon="🔒" label="قفل الأجهزة" onClick={() => {}} color="bg-slate-900" />
              <QuickAction icon="📍" label="طلب موقع" onClick={() => navigate('/map')} color="bg-indigo-600" />
              <QuickAction icon="📶" label="قطع النت" onClick={() => {}} color="bg-red-600" />
              <QuickAction icon="🌙" label="وضع النوم" onClick={() => navigate('/modes')} color="bg-amber-600" />
            </div>
          </div>
