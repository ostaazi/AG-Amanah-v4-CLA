...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  /**
   * خوارزمية توليد الرابط النظيف (Clean Link Generator) - تصحيح v4.3
   * تم الاستغناء عن pathname لمنع تكرار المسارات أو تضمين معرفات جلسات AI Studio.
   */
  const getPortalUrl = (token?: string) => {
    try {
      // استخدام origin فقط (مثل https://example.com)
      const baseUrl = window.location.origin;
      
      // بناء الرابط بشكل نظيف ومباشر متوافق مع Hash Routing
      if (token) {
      ...
      // Fallback آمن في حالة فشل الـ URL API
    ...
    <div className="max-w-4xl mx-auto space-y-10 pb-40 animate-in fade-in" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="space-y-4">
        {children.length > 0 ? children.map(child => (
          ...
          <div className="text-center py-10 text-slate-400 font-bold italic">لا توجد أجهزة مرتبطة حالياً</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-0 md:p-6 bg-slate-950/98 backdrop-blur-3xl overflow-y-auto">
           <div className="bg-white w-full max-w-2xl md:rounded-[4rem] shadow-2xl border-t-8 border-indigo-600 flex flex-col items-center p-6 md:p-14 text-center relative my-auto h-full md:h-auto overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-100 text-right w-full space-y-4">
                   <p className="text-[11px] font-bold text-slate-600">رابط الربط المباشر (للمشاركة اليدوية عبر المتصفح):</p>
              ...
              <button onClick={() => setShowPairingModal(false)} className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] font-black text-xl mt-10 shadow-xl active:scale-95 transition-all">تم الربط بنجاح</button>
           </div>
        </div>
      )}
    </div>
