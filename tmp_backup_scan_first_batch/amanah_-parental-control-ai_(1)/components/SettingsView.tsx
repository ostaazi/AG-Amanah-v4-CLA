...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  // Modern Input Style
  const inputStyle = "w-full p-5 bg-slate-50 border-2 border-transparent rounded-[1.5rem] outline-none focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-700 placeholder:text-slate-300 shadow-sm text-lg";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-32 animate-in fade-in" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <section className="bg-white/80 backdrop-blur-2xl rounded-[3.5rem] p-10 border border-white shadow-xl space-y-10">
        <h3 className="text-2xl font-black text-slate-800 border-b pb-4 flex items-center gap-4">🔐 الأمان والخصوصية</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
           <div className="space-y-6">
              <div className="flex justify-between items-center px-4">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">تغيير كلمة المرور</p>
              </div>
              <div className="relative">
                ...
                <button onClick={() => setShowForgotModal(true)} className="absolute left-6 top-1/2 -translate-y-1/2 text-[10px] font-black text-indigo-600 hover:text-indigo-800 transition-colors bg-indigo-50 px-3 py-1.5 rounded-lg">نسيت كلمة المرور؟</button>
              </div>
              ...
              <button onClick={handlePasswordChange} className="w-full py-4 bg-slate-900 text-white rounded-full font-black text-sm shadow-xl hover:bg-slate-800 active:scale-95 transition-all">حفظ التغييرات</button>
           </div>
           <div className="space-y-6">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">إعدادات الدخول</p>
              <ToggleRow label="المصادقة الثنائية (2FA)" active={twoFactorEnabled} onToggle={handle2FAToggle} lang={lang} />
              <ToggleRow label="البصمة البيومترية" active={biometricsEnabled} onToggle={handleBioToggle} lang={lang} />
           </div>
        </div>
      </section>
      ...
      <section className="bg-slate-900 rounded-[3.5rem] p-10 shadow-2xl space-y-8 text-white border-b-8 border-indigo-600 relative overflow-hidden">
         ...
         <div className="flex justify-between items-center relative z-10">
             <div>
                <h3 className="text-2xl font-black flex items-center gap-3">🛠️ التثبيت الدائم (نسخ الكود)</h3>
         ...
         <div className="space-y-4 relative z-10">
            ...
            >
               ...
               <div className="text-right">
                  <span className="block text-sm">نسخ الكود للحافظة (Copy)</span>
      ...
      <section className="bg-white/80 backdrop-blur-2xl rounded-[3.5rem] p-10 border border-white shadow-xl space-y-8">
         <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-4">
            <div>
                ...
                📸 مكتبة الصور الحية
                <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black border border-indigo-100">{avatarLibrary.length} صورة</span>
                </h3>
                <p className="text-[10px] font-bold text-slate-400 mt-1">
                   رتب وارفع الصور هنا، ثم استخدم زر النسخ أعلاه لتثبيتها في الكود.
                </p>
            </div>
      ...
      <section className="bg-gradient-to-br from-indigo-50 to-white backdrop-blur-xl p-12 rounded-[3.5rem] border border-white shadow-xl space-y-8 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h4 className="text-3xl font-black text-indigo-900 mb-2">إضافة فرد للعائلة</h4>
            <div className="bg-slate-100 p-2 rounded-[2rem] flex mb-8 max-w-md mx-auto shadow-inner">
               <button onClick={() => setNewMemberRole('CHILD')} className={`flex-1 py-4 rounded-[1.8rem] text-sm font-black transition-all flex items-center justify-center gap-2 ${newMemberRole === 'CHILD' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}><span>👶</span> طفل</button>
               <button onClick={() => setNewMemberRole('SUPERVISOR')} className={`flex-1 py-4 rounded-[1.8rem] text-sm font-black transition-all flex items-center justify-center gap-2 ${newMemberRole === 'SUPERVISOR' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}><span>🛡️</span> مشرف</button>
            </div>
            ...
            <button onClick={handleAddNewMember} className="w-full mt-8 py-5 rounded-[2rem] font-black text-white bg-indigo-600 shadow-xl shadow-indigo-200 hover:bg-indigo-700 hover:shadow-2xl hover:-translate-y-1 transition-all active:scale-95 text-lg">إضافة</button>
          </div>
      </section>
             ...
             <div key={member.id} className="flex items-center justify-between p-6 bg-slate-50/50 rounded-[2.5rem] border border-white shadow-sm hover:shadow-md transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${getRoleBadgeColor(member.role)}`}>{member.role === 'ADMIN' ? 'مدير' : member.role === 'SUPERVISOR' ? 'مشرف' : 'طفل'}</span>
                  </div>
                </div>
                <div className="flex gap-2">
      ...
      {showEditMemberModal && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md animate-in fade-in">
           <div className="bg-white w-full max-w-lg rounded-[3rem] p-10 shadow-2xl space-y-8 animate-in zoom-in-95 text-right relative overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar">
