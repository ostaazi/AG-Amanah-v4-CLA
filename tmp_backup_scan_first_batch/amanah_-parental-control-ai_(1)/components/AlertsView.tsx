...
const AlertsView: React.FC<{ alerts: MonitoringAlert[], theme: 'light' | 'dark' }> = ({ alerts, theme }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20 animate-in fade-in duration-700" dir="rtl">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          <h2 className={`text-4xl font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>السجل الأمني الشامل</h2>
          <p className="text-slate-500 font-bold mt-2">مراجعة تاريخية لكافة التهديدات والتدخلات الآلية.</p>
        </div>
      </div>
                ...
                <div className={`${isDark ? 'bg-indigo-950/20' : 'bg-indigo-50/50'} p-8 rounded-[2.5rem] border border-indigo-100 relative overflow-hidden`}>
                   <div className="relative z-10">
                      <div className="flex items-center gap-3 mb-3">
                         ...
                         <h4 className={`text-[11px] font-black uppercase tracking-widest ${isDark ? 'text-indigo-300' : 'text-slate-800'}`}>تحليل Amanah AI</h4>
                      </div>
                      ...
                      <div className={`flex items-center justify-between p-4 rounded-2xl shadow-sm border ${isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-indigo-100'}`}>
                         <div className="flex items-center gap-3">
                            ...
                            <span className={`text-xs font-black ${isDark ? 'text-white' : 'text-slate-800'}`}>الإجراء: <span className="text-red-600 font-black">{alert.actionTaken}</span></span>
                         </div>
                      </div>
                   </div>
                </div>
                ...
                <div className="lg:w-80 w-full h-80 relative bg-slate-100 rounded-[2.5rem] overflow-hidden border-4 border-slate-50 shadow-2xl flex-shrink-0 group">
                   ...
                   {alert.severity === AlertSeverity.CRITICAL && !unblurredImages.has(alert.id) && (
                     <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-red-950/20 backdrop-blur-sm">
                        ...
                        <p className="text-[10px] font-black text-white uppercase tracking-widest bg-red-600 px-4 py-2 rounded-full mb-4">محتوى حساس محجوب</p>
                        <button 
                   ...
                   {unblurredImages.has(alert.id) && (
                     <button 
                        onClick={() => toggleBlur(alert.id)}
                        className="absolute top-4 right-4 bg-black/60 text-white p-3 rounded-2xl backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity"
                     >
                       إعادة الحجب
                     </button>
                   )}
