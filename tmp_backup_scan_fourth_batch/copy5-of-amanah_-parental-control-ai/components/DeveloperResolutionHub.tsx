...
import { SystemPatch, ParentAccount, UserRole } from '../types';
  ...
  useEffect(() => {
    const checkRole = async () => {
      ...
      try {
        ...
        if (parentDoc.exists()) {
          const role = parentDoc.data().role as UserRole;
          const staffRoles: UserRole[] = ['RELEASE_MANAGER', 'DEVELOPER', 'SOC_ANALYST', 'SRE', 'PLATFORM_ADMIN'];
  ...
  if (isAuthorized === false) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center p-10 space-y-6">
         <div className="w-32 h-32 bg-slate-900 text-white rounded-[3rem] flex items-center justify-center text-6xl shadow-inner border border-white/10">🛠️</div>
         <h2 className="text-3xl font-black text-slate-900">مركز التحكم مقيد</h2>
         <p className="text-slate-500 font-bold max-w-md">أدوات النسخ الاحتياطي السيادي وسجلات النواة مخصصة للفريق التقني فقط. يرجى التواصل مع الـ Infrastructure Lead.</p>
      </div>
    );
  }

  if (isAuthorized === null) return <div className="p-20 text-center font-black animate-pulse text-slate-400">Loading Command Center...</div>;
