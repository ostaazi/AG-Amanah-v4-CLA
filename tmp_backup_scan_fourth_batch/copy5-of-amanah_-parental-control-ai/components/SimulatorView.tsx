...
interface SimulatorViewProps {
  /* Fix: Renamed children to childList to avoid conflict with React's reserved children prop */
  childList: Child[];
  parentId: string;
  lang: 'ar' | 'en';
}

const SimulatorView: React.FC<SimulatorViewProps> = ({ childList, parentId, lang }) => {
  ...
  const handleSimulate = async () => {
    ...
    /* Fix: Using childList instead of children */
    const child = childList[0] || { name: 'أحمد' };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 pb-40 animate-in fade-in" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-2xl space-y-6">
        <div onClick={() => !loading && fileInputRef.current?.click()} className={`relative w-full h-48 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all overflow-hidden ${displayImage ? 'border-indigo-50 bg-slate-900' : 'border-slate-300 bg-slate-50'}`}>
