...
const ChildAppView: React.FC<ChildAppViewProps> = ({ childName, lang }) => {
  ...
  return (
    <div className="min-h-screen bg-indigo-950 flex flex-col items-center justify-center p-8 text-center text-white" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-10 space-y-12 animate-in fade-in duration-1000">
        ...
        <div className="grid grid-cols-1 gap-6 w-full max-w-sm mx-auto">
           <div className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] backdrop-blur-md">
              <div className="flex items-center justify-between mb-4">
                 <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">حالة الاتصال</span>
           ...
           <div className="grid grid-cols-4 gap-4">
              {['📚', '🎓', '📝', '📞'].map((icon, idx) => (
                <div key={idx} className="w-full aspect-square bg-indigo-900/40 rounded-2xl flex items-center justify-center text-3xl border border-indigo-500/20 shadow-lg">
                  {icon}
                </div>
              ))}
           </div>
        ...
        <div className="pt-10">
           <div className="flex items-center justify-center gap-3 text-white/40">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em]">Amanah Protocol v1.0.4</span>
              <div className="w-2 h-2 bg-white/20 rounded-full"></div>
           </div>
        </div>
      </div>
