...
interface SettingsViewProps {
  ...
  /* Fix: Renamed children to childList to avoid collision with React's reserved children prop */
  childList: Child[];
  lang: 'ar' | 'en';
...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  currentUser, childList, lang, onUpdateMember, onDeleteMember, showSuccessToast 
}) => {
  ...
  const loadSupervisors = async () => {
    // Fix: fetchSupervisors correctly typed as returning FamilyMember[]
  ...
  const toggleRole = async (member: any) => {
    // Fix: Added roles that exist in UserRole enum
    ...
    setIsProcessing(true);
    // Fix: Passing UserRole typed role
            ...
            <div key={member.id} className="p-6 bg-white rounded-[2.5rem] border border-slate-100 flex items-center justify-between shadow-sm group">
               <div className="flex items-center gap-6">
                  <div className="relative">
                    ...
                    {(member.role === 'ADMIN' || member.role === 'FAMILY_OWNER') && <div className="absolute -bottom-2 -left-2 w-8 h-8"><AdminShieldBadge /></div>}
                  </div>
                  <div>
