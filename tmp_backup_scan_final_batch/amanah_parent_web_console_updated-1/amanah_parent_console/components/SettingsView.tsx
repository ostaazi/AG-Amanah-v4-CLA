...
import { generate2FASecret, getQRCodeDataUrl, toQRCodeDataUrl, verifyTOTP } from '../services/twoFAService';
...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const [twoFAQrDataUrl, setTwoFAQrDataUrl] = useState<string>('');
  ...
  // Generate 2FA QR locally (no third-party requests) when needed.
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      if (!showTwoFAModal || twoFAStep !== 'QR' || !tempSecret) return;
      try {
        const url = await getQRCodeDataUrl(currentUser.email || 'user', tempSecret);
        if (!cancelled) setTwoFAQrDataUrl(url);
      } catch (e) {
        console.error('QR generation error', e);
        if (!cancelled) setTwoFAQrDataUrl('');
      }
    };
    run();
    return () => { cancelled = true; };
  }, [showTwoFAModal, twoFAStep, tempSecret, currentUser.email]);

  // Generate pairing QR locally as well.
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      if (!showPairingModal?.id) {
        setPairingQrDataUrl('');
        return;
      }
      try {
        const payload = `amanah-pair-${showPairingModal.id}`;
        const url = await toQRCodeDataUrl(payload, 200);
        if (!cancelled) setPairingQrDataUrl(url);
      } catch (e) {
        console.error('Pairing QR generation error', e);
  ...
  const handleInjectMockData = async () => {
    ...
    try {
      const mod = await import('../services/mockDataService');
      await mod.injectMockSuite(currentUser.id);
  ...
  const handleClearData = async () => {
    ...
    try {
      const mod = await import('../services/mockDataService');
      await mod.clearAllUserData(currentUser.id);
         ...
         <div className="relative z-10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               ...
               <button onClick={async () => {
                 if (!import.meta.env.DEV) return;
                 const mod = await import('../services/mockDataService');
                 await mod.randomizePsychProfiles(currentUser.id);
               }} className="py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black text-xs shadow-xl active:scale-95 transition-all hover:bg-indigo-500">🧠 تحديث النبض النفسي</button>
                 ...
                 {twoFAStep === 'QR' && (
                   <div className="space-y-10 animate-in fade-in zoom-in-95">
                      <div className="bg-white p-6 rounded-[3rem] shadow-inner border-2 border-slate-50 flex justify-center inline-block mx-auto relative group">
                         {twoFAQrDataUrl ? (
                           <img src={twoFAQrDataUrl} className="w-56 h-56 transition-transform group-hover:scale-105 duration-500" />
                         ) : (
                           <div className="w-56 h-56 rounded-[2.5rem] bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 font-black text-xs">
                             جاري إنشاء QR...
                           </div>
                         )}
        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-2xl">
           <div className="bg-white w-full max-w-md rounded-[3.5rem] p-10 text-center animate-in zoom-in-95 shadow-2xl">
              ...
              <div className="bg-white p-8 rounded-[3rem] mb-8 shadow-inner border border-slate-50 flex justify-center group">
                 {pairingQrDataUrl ? (
                   <img src={pairingQrDataUrl} className="w-48 h-48 group-hover:scale-105 transition-transform duration-500" />
                 ) : (
                   <div className="w-48 h-48 rounded-[2.5rem] bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 font-black text-xs">
                     QR...
                   </div>
                 )}
              </div>
