...
interface RoleConfig {
  // Fix: Change type to allow wildcard strings like 'device.*' and ensure compatibility with AppPermission union
  allow: (AppPermission | string)[];
  deny: (AppPermission | string)[];
}
...
const ROLE_MATRIX: Record<UserRole, RoleConfig> = {
  ...
  'SOC_ANALYST': {
    ...
    deny: ['evidence.*', 'realtime.*', 'child.*', 'device.*']
  },
  'SRE': {
    allow: ['*'], 
  ...
  // Added PARENT_OWNER and PARENT_GUARDIAN to satisfy Record<UserRole, RoleConfig>
  'PARENT_OWNER': {
    allow: ['*'],
    deny: []
  },
...
export const hasPermission = (role: UserRole, permission: AppPermission): boolean => {
  ...
  const denies = config.deny as string[];
  const allows = config.allow as string[];
  const parts = permission.split('.');
  
  // Fix: Improved logic to support hierarchical wildcard checking for both allow and deny lists
  // Check for denials first
  if (denies.includes('*') || denies.includes(permission)) return false;
  for (let i = 1; i < parts.length; i++) {
    const pattern = parts.slice(0, i).join('.') + '.*';
    if (denies.includes(pattern)) return false;
  }

  // Check for allows
  if (allows.includes('*') || allows.includes(permission)) return true;
  for (let i = 1; i < parts.length; i++) {
    const pattern = parts.slice(0, i).join('.') + '.*';
    if (allows.includes(pattern)) return true;
  }

  return false;
};

export const canPerform = (role: UserRole, permission: AppPermission) => hasPermission(role, permission);