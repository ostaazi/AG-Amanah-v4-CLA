...
export const analyzeLocationSafety = async (lat: number, lng: number) => {
  ...
  if (!ai) {
    return { text: "مفتاح Gemini غير مُعدّ. أضف GEMINI_API_KEY إلى Secrets في Space.", mapsLinks: [] };
  }
...
export const analyzeContent = async (
  ...
  if (!ai) {
    return {
      ...
      aiAnalysis: "Gemini API key is not configured for this deployment.",
      actionTaken: "Add GEMINI_API_KEY (or VITE_GEMINI_API_KEY) to your Space Secrets and restart.",
      suspectUsername: "unknown",
      conversationLog: []
    };
  }
