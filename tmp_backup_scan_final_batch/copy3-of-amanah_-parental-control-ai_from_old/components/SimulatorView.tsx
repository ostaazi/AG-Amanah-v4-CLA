...
const SimulatorView: React.FC<SimulatorViewProps> = ({ children, parentId, lang }) => {
  ...
  const finalizeAlert = async (child: any, content: string, category: Category, severity: AlertSeverity, lat: string, analysis: string, action: string, suspect?: string, log?: any[]) => {
      ...
      // تأكد من أن جميع البيانات هي Plain Data لتجنب Circular Reference
      const alertData: any = {
        ...
        conversationLog: log && Array.isArray(log) ? log.map(m => ({
            ...
            { sender: String(child?.name || 'الهدف'), text: "لا أعرفك!", time: new Date().toLocaleTimeString(), isSuspect: false }
        ]
      };
  ...
  return (
    <div className="max-w-xl mx-auto space-y-8 pb-40 animate-in fade-in" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-2xl space-y-6">
        ...
        <textarea rows={2} value={text} onChange={(e) => setText(e.target.value)} placeholder="نص الرسالة..." className="w-full p-6 rounded-[2rem] bg-slate-50 border-2 border-slate-100 outline-none font-bold text-right" />

        <div className="grid grid-cols-3 gap-2">
            {(lang === 'ar' ? AR_SCENARIOS : EN_SCENARIOS).map((s) => (
                <button key={s.id} onClick={() => setText(s.text)} className={`py-3 px-2 rounded-xl text-white text-[10px] font-black shadow-md ${s.color}`}>{s.label}</button>
            ))}
        </div>
        
        <button onClick={handleSimulate} disabled={loading || (!text.trim() && !displayImage)} className="w-full py-6 bg-indigo-600 text-white rounded-[2.5rem] font-black text-lg active:scale-95 shadow-xl">
