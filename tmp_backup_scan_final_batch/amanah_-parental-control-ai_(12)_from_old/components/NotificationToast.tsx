...
const NotificationToast: React.FC<NotificationToastProps> = ({ alert, onClose }) => {
  ...
  const handleToastClick = () => {
    // الانتقال للخزنة وفتح ملف الأدلة لهذا التنبيه تحديداً
  ...
  const handleCloseIcon = (e: React.MouseEvent) => {
    e.stopPropagation(); // منع تفعيل الضغط على التنبيه عند الرغبة في الإغلاق فقط
    setVisible(false);
    setTimeout(onClose, 500);
  };
  ...
  return (
    <div className={`fixed top-8 left-0 right-0 z-[9999] px-4 transition-all duration-500 ease-out transform ${visible ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0'}`}>
      <div 
        ...
        className="max-w-md mx-auto bg-white/95 backdrop-blur-xl rounded-[1.5rem] shadow-2xl border border-slate-200 overflow-hidden flex items-center p-4 gap-4 cursor-pointer hover:bg-slate-50 transition-colors group active:scale-[0.98]"
      >
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110 ${alert.severity === AlertSeverity.CRITICAL ? 'bg-red-600 text-white' : 'bg-indigo-600 text-white'}`}>
        ...
        <div className="flex-1 min-w-0 text-right">
          ...
          <p className="text-[8px] font-black text-indigo-500 uppercase mt-1 opacity-0 group-hover:opacity-100 transition-opacity">إضغط لعرض الدليل 📂</p>
        </div>
