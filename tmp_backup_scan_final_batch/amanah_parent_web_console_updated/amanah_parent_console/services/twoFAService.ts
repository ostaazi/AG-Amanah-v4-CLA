...
export const getOtpauthUrl = (email: string, secret: string) => {
  const issuer = "AmanahAI";
  const safeEmail = encodeURIComponent(String(email || "user").trim());
  const label = `${issuer}:${safeEmail}`;
  return `otpauth://totp/${label}?secret=${secret}&issuer=${encodeURIComponent(issuer)}`;
};

/**
 * Generate QR code locally (NO third-party request) to avoid leaking the TOTP secret.
 */
export const getQRCodeDataUrl = async (email: string, secret: string): Promise<string> => {
...
/**
 * Generic QR generator for non-secret payloads (e.g., pairing codes).
 */
export const toQRCodeDataUrl = async (payload: string, width: number = 200): Promise<string> => {
