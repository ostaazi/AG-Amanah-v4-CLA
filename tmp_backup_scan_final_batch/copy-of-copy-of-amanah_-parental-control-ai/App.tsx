...
const App: React.FC = () => {
  ...
  const handleApplyMode = async (childId: string, modeId?: string) => {
    // UI-only placeholder: apply mode without touching env/DB config.
    // If you later want persistence, we can wire it through existing firestoreService functions.
    console.log('Apply mode', { childId, modeId });
  };
    ...
    <div className="h-screen w-full relative overflow-hidden bg-[#f8fafc]" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <main className="scroll-viewport custom-scrollbar pt-28 px-4 md:px-6">
        <Routes>
          ...
          <Route path="/evidence-vault" element={<EvidenceVaultView records={alerts as any} currentUser={currentUser} />} />
          <Route path="/map" element={children.length > 0 ? <MapView children={children} /> : null} />
          <Route path="/modes" element={<ModesView modes={customModes} children={children} onUpdateModes={setCustomModes} onApplyMode={handleApplyMode} />} />
          <Route path="/insights" element={children.length > 0 ? <PsychologicalInsightView theme="light" child={children[0]} onAcceptPlan={(plan) => navigate('/modes', { state: { suggestedMode: plan } })} /> : null} />
...
const Sidebar: React.FC<{ isOpen: boolean, onClose: () => void, lang: 'ar' | 'en' }> = ({ isOpen, onClose, lang }) => {
  ...
  const menuItems = [
    ...
    { path: '/live', label: 'المراقبة المباشرة', icon: <ICONS.LiveCamera /> },
    { path: '/evidence-vault', label: 'خزنة الأدلة', icon: <ICONS.Vault /> },
    { path: '/map', label: 'الخريطة', icon: <ICONS.Location /> },
    { path: '/modes', label: 'الأوضاع', icon: <ICONS.WalkieTalkie /> },
    { path: '/insights', label: 'رؤى نفسية', icon: <ICONS.Pulse /> },
      ...
      <div className={`absolute top-0 bottom-0 w-[80vw] max-w-[300px] bg-white shadow-2xl transition-transform duration-500 p-8 flex flex-col ${lang === 'ar' ? (isOpen ? 'right-0' : 'right-[-300px]') : (isOpen ? 'left-0' : 'left-[-300px]')}`}>
        <div className="flex items-center gap-3 mb-8">
          <div className="w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center shadow-lg">
            <AmanahShield className="w-7 h-7 text-white" />
          </div>
          <div className="leading-tight">
            <div className="text-xs font-black tracking-widest text-slate-400 uppercase">AMANA</div>
            <div className="text-sm font-black text-slate-900">Amanah</div>
          </div>
        </div>
