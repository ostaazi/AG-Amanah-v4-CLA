...
/**
 * Utility to ensure data is a plain object for React/JSON safety
 */
const sanitizeData = (data: any) => {
    const sanitized = { ...data };
    // Convert Firestore Timestamps to native Dates immediately
...
export const fetchSupervisors = async (parentId: string): Promise<FamilyMember[]> => {
    ...
    try {
        ...
        return snap.docs.map(d => ({ id: d.id, ...sanitizeData(d.data()) } as any));
    } catch (e) {
...
export const fetchChildrenForParent = async (parentId: string): Promise<Child[]> => {
    ...
    return querySnapshot.docs.map(doc => ({ id: doc.id, ...sanitizeData(doc.data()) } as any));
};
