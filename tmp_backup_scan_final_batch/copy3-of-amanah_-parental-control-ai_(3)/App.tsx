...
const App: React.FC = () => {
  ...
  if (isAuthChecking) return (
    <div className="min-h-screen bg-[#050510] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      <AmanahGlobalDefs />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(138,21,56,0.1)_0%,transparent_70%)] animate-pulse"></div>
      <div className="w-full max-w-[320px] relative z-20 flex flex-col items-center animate-in zoom-in-90 duration-1000">
        <AmanahLogo className="w-full h-auto drop-shadow-2xl" />
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-['Cairo'] relative overflow-x-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className="fixed top-8 left-0 right-0 h-20 px-6 flex items-center justify-between z-[150] bg-white/70 backdrop-blur-2xl border-b border-white shadow-sm">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/settings')}>
           <div className="relative">
             <img src={currentUser.avatar} className="w-14 h-14 rounded-full object-cover border-2 border-white shadow-md" />
             <div className="absolute bottom-0 -left-1 w-6 h-6">
                <AdminShieldBadge />
             </div>
           </div>
           <div className="hidden sm:block">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t.admin}</p>
              <p className="text-sm font-black text-slate-800 leading-tight">{currentUser.name}</p>
           </div>
        </div>
        
        <div className="absolute left-1/2 -translate-x-1/2 w-20 md:w-24 drop-shadow-sm cursor-pointer" onClick={() => navigate('/')}>
           <AmanahLogo />
        </div>

        <div className="flex items-center gap-3">
            <button onClick={() => setIsMenuOpen(true)} className="p-4 bg-white shadow-lg rounded-2xl border border-slate-50 text-slate-700 active:scale-90"><ICONS.Menu /></button>
        </div>
      </header>

      <main className="pt-44 px-4 pb-48 max-w-7xl mx-auto min-h-screen">
        <Routes>
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-3xl border-t border-slate-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-[9999] pb-4 pt-2 flex flex-col items-center">
         <div className="flex overflow-x-auto custom-scrollbar items-center justify-start sm:justify-center gap-2 sm:gap-4 px-6 w-full max-w-3xl">
           {menuItems.map((item) => (
...
const Sidebar: React.FC<{ isOpen: boolean, onClose: () => void, lang: 'ar' | 'en', menuItems: any[] }> = ({ isOpen, onClose, lang, menuItems }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[6000] transition-all duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className={`absolute top-0 bottom-0 w-80 bg-white shadow-2xl transition-transform duration-500 p-10 flex flex-col ${lang === 'ar' ? (isOpen ? 'right-0' : 'right-[-100%]') : (isOpen ? 'left-0' : 'left-[-100%]')}`}>
        <div className="flex justify-between items-center mb-16">
          <div className="w-24"><AmanahLogo /></div>
          <button onClick={onClose} className="p-3 bg-slate-50 rounded-full text-slate-400"><ICONS.Close /></button>
        </div>
        <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => (
            <button key={item.path} onClick={() => { navigate(item.path); onClose(); }} className="w-full flex items-center gap-6 p-5 rounded-[2rem] hover:bg-indigo-50 transition-all text-right group">
        ...
        <div className="pt-8 border-t border-slate-100 mt-6">
           <button onClick={() => { logoutUser(); onClose(); }} className="w-full py-5 bg-red-50 text-red-600 rounded-2xl font-black text-xs uppercase tracking-widest">تسجيل الخروج</button>
        </div>
      </div>
    </div>
  );
...
const NavLink: React.FC<{ to: string, icon: any, label: string }> = ({ to, icon, label }) => {
  ...
  return (
    <button onClick={() => navigate(to)} className={`flex flex-col items-center justify-center min-w-[65px] gap-0.5 transition-all group shrink-0 ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
      <div className={`p-2 rounded-xl transition-all ${isActive ? 'bg-indigo-100 shadow-sm' : 'bg-transparent group-hover:bg-slate-50'}`}>
        <div className={`${isActive ? 'scale-110' : 'scale-100'} transition-transform`}>{icon}</div>
      </div>
      <span className={`text-[9px] font-black whitespace-nowrap transition-all uppercase tracking-tighter ${isActive ? 'opacity-100' : 'opacity-60'}`}>{label}</span>
    </button>
  );
};
