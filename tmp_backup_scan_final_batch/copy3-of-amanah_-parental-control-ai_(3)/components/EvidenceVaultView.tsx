      ...
      {selectedRecord && (
        <div className="fixed inset-0 z-[8000] flex items-center justify-center p-0 md:p-6 bg-slate-950/95 backdrop-blur-2xl animate-in fade-in duration-300 print:relative print:inset-auto print:bg-white overflow-y-auto custom-scrollbar">
          ...
          <div className="bg-white w-full max-w-2xl h-full md:h-auto md:max-h-[96vh] md:rounded-[4rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col animate-in zoom-in-95 print:shadow-none print:rounded-none">
            ...
            <div className="bg-[#050510] px-8 py-6 flex justify-between items-center flex-shrink-0 z-[8100] border-b border-white/5 relative shadow-xl">
               <button onClick={() => setSelectedRecord(null)} className="text-white hover:text-red-500 transition-colors p-2 bg-white/5 rounded-full border border-white/10 shadow-inner">
            ...
            {/* Actions Controls - التعديل النهائي لمطابقة القصاصة المرفقة بدقة */}
            <div className="px-6 py-6 pb-64 md:pb-12 bg-white border-t border-slate-100 flex items-center justify-center gap-4 flex-shrink-0 z-[8200] print:hidden">
               
               {/* 📄 تصدير للبلاغ (Rightmost visually in snippet) */}
               <button 
                 onClick={handleExport} 
                 className="flex-1 h-16 bg-[#000000] text-white rounded-[1.8rem] flex items-center justify-center gap-4 transition-all active:scale-95 shadow-xl hover:bg-zinc-900"
               >
                  <div className="bg-[#dc2626] text-white text-[10px] px-2 py-0.5 rounded-md font-black tracking-widest leading-normal">
                    PDF
                  </div>
               ...
               <button 
                 onClick={handleSave} 
                 className="flex-1 h-16 bg-[#10a173] text-white rounded-[1.8rem] flex items-center justify-center gap-4 transition-all active:scale-95 shadow-lg shadow-emerald-100 hover:bg-[#0d8f66]"
               >
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center border border-white/20">
                    <AmanahShield className="w-6 h-6 opacity-70" />
                  </div>
                  <span className="text-lg font-black tracking-tight">حفظ كدليل</span>
               </button>

               {/* 🗑️ إتلاف السجل (Leftmost visually in snippet) */}
               <button 
                 onClick={handleDelete} 
                 className="flex-1 h-16 bg-[#fff3f4] text-[#b22c2c] rounded-[1.8rem] flex items-center justify-center gap-4 transition-all active:scale-95 border border-red-50 hover:bg-[#ffeaea]"
               >
