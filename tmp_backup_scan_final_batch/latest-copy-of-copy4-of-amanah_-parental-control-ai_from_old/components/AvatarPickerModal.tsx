...
const AvatarPickerModal: React.FC<AvatarPickerModalProps> = ({ isOpen, onClose, onSelect, currentAvatar, title }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in" dir="rtl">
      <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-[85vh] animate-in zoom-in-95">
        <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50 flex-shrink-0">
           <div className="text-right">
              <h3 className="text-2xl font-black text-slate-800 tracking-tighter">{title || 'تغيير الصورة الشخصية'}</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">اختر من مكتبة أمانة ({allAvatars.length} صورة)</p>
           </div>
           <button onClick={onClose} className="p-3 hover:bg-slate-200 rounded-full transition-all text-slate-400"><ICONS.Close /></button>
        </div>
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar space-y-10 flex-1 bg-white">
          ...
          <div className="flex flex-col items-center gap-4">
             ...
             >
                <div className="text-5xl group-hover:scale-110 transition-transform">{isUploading ? '⏳' : '📸'}</div>
                <div className="text-center">
                  <span className="font-black text-indigo-600 text-sm block">{isUploading ? 'جاري معالجة الصورة...' : 'رفع صورة من معرض الصور الخاص بك'}</span>
                  <span className="text-[9px] font-bold text-slate-400">يمكنك استخدام الكاميرا أو الملفات</span>
                </div>
                  ...
                  >
                    ...
                    {currentAvatar === url && (
                      <div className="absolute inset-0 bg-indigo-600/20 flex items-center justify-center">
                         <div className="bg-white rounded-full p-1.5 shadow-xl text-[12px] animate-bounce">✅</div>
                      </div>
                    )}
                  </button>
                ))}
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-center gap-4 flex-shrink-0">
           <button onClick={onClose} className="px-14 py-4 bg-white text-slate-500 rounded-2xl font-black text-sm border border-slate-200 shadow-sm hover:bg-slate-100 transition-colors">إغلاق النافذة</button>
        </div>
      </div>
    </div>
  );
