...
const EmergencyOverlay: React.FC<EmergencyOverlayProps> = ({ alert, onClose, onAction, onMinimize }) => {
      ...
      <div className="relative w-full max-w-2xl h-full md:h-auto md:max-h-[96vh] bg-white rounded-none md:rounded-[3.5rem] shadow-[0_0_150px_rgba(220,38,38,0.4)] overflow-y-auto custom-scrollbar flex flex-col border-0 md:border-4 border-white animate-in zoom-in-95 duration-300">
           ...
           <div className="flex justify-between items-start mb-8">
              <div className="flex items-center gap-5">
                 ...
                 <div className="text-right">
                    <h2 className="text-4xl font-black tracking-tighter leading-none mb-1 text-white">تنبيه طوارئ حرج</h2>
                    <p className="text-red-100 text-sm font-bold opacity-80">رصد محاولة استدراج أو تهديد مباشر</p>
                 </div>
              </div>
              <div className="flex gap-2">
                {onMinimize && (
                   <button onClick={onMinimize} className="p-3 bg-white/10 rounded-full border border-white/20 hover:bg-white/20 transition-all flex items-center justify-center" title="تصغير الإشعار">
                      <span className="text-lg">🤏</span>
                   </button>
                )}
                <button onClick={onClose} className="p-3 bg-white/10 rounded-full border border-white/20 hover:bg-white/20 transition-all flex items-center justify-center">
                   <ICONS.Close />
                </button>
              </div>
           </div>
        ...
        <div className="p-8 md:p-12 bg-white border-t border-slate-100 flex-shrink-0 pb-32 md:pb-12 space-y-4">
           <button onClick={onAction} className="w-full h-24 bg-slate-900 text-white rounded-[2.5rem] font-black text-xl flex items-center justify-center gap-8 shadow-2xl active:scale-95 transition-all">
              ...
              <div className="text-right">
                 <p className="leading-none mb-2 text-white">فتح تفاصيل الدليل الجنائي</p>
                 <p className="text-xs font-bold text-slate-400 font-mono tracking-widest uppercase">Secure_Vault_Access</p>
              </div>
           </button>
           
           {onMinimize && (
             <button onClick={onMinimize} className="w-full h-14 bg-slate-100 text-slate-600 rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-slate-200 transition-all active:scale-95">
                <span>🔔</span>
                تبسيط التنبيه لإشعار علوي فقط
             </button>
           )}
        </div>
      </div>
