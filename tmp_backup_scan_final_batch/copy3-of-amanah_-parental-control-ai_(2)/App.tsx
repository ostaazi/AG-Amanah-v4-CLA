...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-['Cairo'] relative overflow-x-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* Header Container */}
      <header className="fixed top-7 left-0 right-0 h-28 px-6 flex items-center justify-between z-[150] bg-white/60 backdrop-blur-xl border-b border-white/20 shadow-sm">
        <div className="flex items-center gap-6 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate('/settings')}>
           <div className="relative">
             {/* جعل الأفاتار شفافاً بالكامل بدون خلفية أو حدود */}
             <img src={currentUser.avatar} className="w-20 h-20 rounded-full object-cover bg-transparent" />
             {/* جعل حاوية البادج شفافة أيضاً وبدون حدود أو ظل */}
             <div className="absolute bottom-0 -left-1 w-8 h-8 rounded-full flex items-center justify-center bg-transparent">
                <AdminShieldBadge className="w-full h-full" />
             </div>
           </div>
           <div className="hidden sm:block">
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{translations[lang].admin}</p>
              <p className="text-lg font-black text-slate-800 leading-tight">{currentUser.name}</p>
           </div>
        </div>
      ...
      {/* Main Content */}
      <main className="pt-44 px-4 pb-52 max-w-7xl mx-auto relative z-[50] min-h-screen">
        <Routes>
