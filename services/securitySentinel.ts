...
const RAW_DANGER_DATA = [
  ...
  {
    words: ['غبي', 'حيوان', 'حقير', 'كلب', 'فاشل', 'stupid', 'hate', 'loser', 'idiot', 'bitch'],
    category: Category.BULLYING,
    severity: AlertSeverity.MEDIUM,
  },
  {
    words: ['ابتزاز', 'بفضحك', 'بنشر', 'صورك', 'بفضح', 'blackmail', 'leak', 'shame'],
  ...
  for (const group of PRE_INDEXED_DICTIONARY) {
    for (const wordSkeleton of group.skeletons) {
      if (inputSkeleton.includes(wordSkeleton)) {
        const hasAction = PRE_INDEXED_ACTIONS.some((aSkeleton) =>
          inputSkeleton.includes(aSkeleton)
        );
