  ...
  files.forEach(file => {
    ...
    if (file.content.includes('"+u+"') || file.content.includes("SELECT * FROM users")) {
      vulnerabilities.push({
        ...
        severity: 'CRITICAL',
        description: 'يتم دمج مدخلات المستخدم مباشرة في استعلام القاعدة، مما يسمح باختراق الحسابات.',
        originalCode: file.content,
        fixedCode: 'export const login = async (u, p) => {\n  return await signInWithEmailAndPassword(auth, u, p);\n};',
        fixExplanation: 'استخدام الـ SDK الرسمي بدلاً من الاستعلامات النصية المباشرة يؤمن العملية بنسبة 100%.'
      });
      score -= 30;
    }
    ...
    if (file.content.includes('innerHTML') || file.content.includes('document.body')) {
      vulnerabilities.push({
        ...
        fixExplanation: 'استخدام خاصية الإدراج النصي الآمن في React تمنع تنفيذ السكريبتات تلقائياً.'
      });
      score -= 30;
    }
    ...
    if (file.content.includes('AIzaSyD3pZgmPyzMh') && file.path.includes('config')) {
      vulnerabilities.push({
        ...
        fixExplanation: 'يجب تخزين المفاتيح في متغيرات البيئة (Environment Variables) بعيداً عن الكود.'
      });
      score -= 40;
    }
  });
