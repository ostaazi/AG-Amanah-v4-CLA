 ...
 */
const detectInjuriesAdvanced = (
  imgElement: HTMLImageElement
): { isDanger: boolean; score: number; type: string } => {
  try {
      ...
      for (let col = 0; col < gridCols; col++) {
        ...
        for (let y = 0; y < gridSize; y++) {
          for (let x = 0; x < gridSize; x++) {
            ...
            const isDeepInjury =
              (hsv.h < 30 || hsv.h > 300) && hsv.s > 25 && hsv.v < 35 && hsv.v > 2;
...
export const scanImageLocally = async (
  imgElement: HTMLImageElement
  ...
  try {
    ...
    if (sexy && sexy.probability > 0.75) {
      return {
        ...
        probability: sexy.probability,
        label: 'NSFW: Suggestive',
        latency: duration,
      };
    }
