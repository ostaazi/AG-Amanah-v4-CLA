...
const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  useEffect(() => {
    const checkRotation = async () => {
      ...
      if (!currentUser.pairingKey || !expires || now > expires) {
        ...
        try {
          await rotatePairingKey(currentUser.id);
        } catch (e) {
  ...
  const handleRegenerateKey = async () => {
    ...
    try {
      ...
      if (err?.code === 'permission-denied') {
        showSuccessToast('فشل التحديث: تأكد من نشر Firestore Rules وتسجيل الدخول بالحساب الأب');
      } else {
        showSuccessToast('تعذر تحديث مفتاح الربط');
      }
    } finally {
  ...
  const updateProtocol = async (mode: AlertProtocolMode) => {
    ...
    const msg =
      mode === 'FULL'
        ? lang === 'ar'
          ? 'ØªÙ… ØªÙØ¹ÙŠÙ„ ÙˆØ¶Ø¹ Ø´Ø§Ø´Ø© Ø§Ù„Ø·ÙˆØ§Ø±Ø¦'
          : 'Emergency screen enabled'
        : mode === 'SIMPLE'
          ? lang === 'ar'
            ? 'ØªÙ… ØªÙØ¹ÙŠÙ„ Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª Ø§Ù„Ø¨Ø¨Ø³ÙŠØ·Ø©'
            : 'Simple notifications enabled'
          : lang === 'ar'
            ? 'ØªÙ… ØªÙØ¹ÙŠÙ„ Ø§Ù„ÙˆØ¶Ø¹ Ø§Ù„ØµØ§Ù…Øª'
            : 'Silent mode enabled';
  ...
  const handleAddChildProfile = async () => {
    ...
    try {
      ...
      showSuccessToast(
        lang === 'ar' ? 'ØªÙ…Øª Ø¥Ø¶Ø§ÙØ© Ù…Ù„Ù Ø§Ù„Ø·ÙÙ„ Ø¨Ù†Ø¬Ø§Ø­' : 'Child profile added successfully'
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  const handleVerify2FA = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        showSuccessToast(
          lang === 'ar' ? 'ØªÙ… ØªÙØ¹ÙŠÙ„ Ø§Ù„Ù…ØµØ§Ø¯Ù‚Ø© Ø§Ù„Ø«Ù†Ø§Ø¦ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­!' : '2FA enabled successfully!'
        );
  ...
  const handlePurgeData = async () => {
    const confirmed = window.confirm(lang === 'ar' ? 'âš ï¸ Ø­Ø°Ù ÙƒØ§ÙØ© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§ØªØŸ' : 'Purge all data?');
    if (!confirmed) return;
    setIsProcessing(true);
  ...
  return (
    ...
    >
        ...
        <div className="flex justify-between items-end px-4">
          <div>
            ...
            <p className="text-slate-400 font-bold text-xs mt-1">
              Ø§Ø¶ØºØ· Ø¹Ù„Ù‰ ØµÙˆØ±Ø© Ø§Ù„Ø·ÙÙ„ Ù„ØªØºÙŠÙŠØ± Ø§Ù„Ø£ÙØ§ØªØ§Ø± Ø§Ù„Ø®Ø§Øµ Ø¨Ù‡.
            </p>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Ø¨Ø·Ø§Ù‚Ø© Ø¥Ø¶Ø§ÙØ© Ø§Ø¨Ù† Ø¬Ø¯ÙŠØ¯ */}
          <div className="bg-indigo-50/50 p-6 rounded-[2.5rem] border-2 border-dashed border-indigo-200 flex flex-col gap-4">
            ...
            >
              {lang === 'ar' ? 'Ø¥Ø¶Ø§ÙØ© Ø§Ø¨Ù† Ø¬Ø¯ÙŠØ¯' : 'Add Child'}
            </button>
          </div>
      ...
      {/* 2. Ù‚Ø³Ù… Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø´Ø±ÙÙŠÙ† */}
      <section className="space-y-6">
        <div className="flex justify-between items-end px-4">
          <div>
      ...
      {/* 2.5 Ù‚Ø³Ù… ØªÙØ¹ÙŠÙ„ Ø§Ù„Ù…Ù…ÙŠØ²Ø§Øª (Feature Toggles) */}
      <section className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl space-y-8">
        <div className="flex justify-between items-center border-b border-slate-50 pb-6">
          <div className="text-right">
            ...
            <p className="text-slate-400 font-bold text-xs">ØªÙØ¹ÙŠÙ„ Ø£Ùˆ ØªØ¹Ø·ÙŠÙ„ ÙˆØ¸Ø§Ø¦Ù Ø§Ù„Ù†Ø¸Ø§Ù… Ø§Ù„Ø£Ø³Ø§Ø³ÙŠØ©.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-[2rem]">
            ...
            <div className="flex items-center gap-3">
              <span className="font-black text-slate-700 text-xs text-right">Ø§Ù„ØªØ­Ù„ÙŠÙ„ Ø§Ù„Ù†ÙØ³ÙŠ</span>
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-[2rem]">
            ...
            <div className="flex items-center gap-3">
              <span className="font-black text-slate-700 text-xs text-right">ÙÙ„ØªØ±Ø© Ø§Ù„ÙˆÙŠØ¨</span>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-[2rem]">
            ...
            <div className="text-right flex items-center gap-4">
              <div>
                <h4 className="font-black text-slate-800 text-sm">ØªÙ†Ø¨ÙŠÙ‡Ø§Øª ÙÙˆØ±ÙŠØ© (Push)</h4>
                <p className="text-[10px] font-bold text-slate-400">Ø§Ø³ØªÙ‚Ø¨Ø§Ù„ ØªÙ†Ø¨ÙŠÙ‡Ø§Øª Ø§Ù„ØªÙ‡Ø¯ÙŠØ¯Ø§Øª ÙÙˆØ±ÙŠØ§Ù‹</p>
              </div>
              ...
              <div className="w-full space-y-3">
                ...
                <div className="bg-black/40 rounded-2xl border border-white/10 p-5 space-y-4 relative">
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase">Ù†Ø³Ø® Ø§Ù„Ù…ÙØªØ§Ø­</span>
                  </button>
                </div>
              </div>
      ...
      <section className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="font-black text-[10px]">Ø¥Ø´Ø¹Ø§Ø±Ø§Øª ÙÙ‚Ø·</p>
          </button>
          <button
      ...
      {/* 5. Ù…ÙØªØ§Ø­ Ø§Ù„Ø±Ø¨Ø· (Dynamic & QR) */}
      <section className="bg-slate-900 rounded-[3rem] p-8 text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col items-center text-center gap-6">
          ...
          <div>
            <h3 className="text-2xl font-black text-[#D1A23D]">Ù…ÙØªØ§Ø­ Ø§Ù„Ø±Ø¨Ø·</h3>
            <p className="text-slate-400 font-bold text-xs mt-1">Ø§Ù…Ø³Ø­ Ø§Ù„Ø±Ù…Ø² Ø£Ùˆ Ø£Ø¯Ø®Ù„ Ø§Ù„ÙƒÙˆØ¯ Ù„Ø±Ø¨Ø· Ø¬Ù‡Ø§Ø² Ø§Ù„Ø·ÙÙ„.</p>
          </div>
      ...
      <section className="bg-red-50 rounded-[2.5rem] p-8 border-2 border-dashed border-red-200 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="text-right">
          ...
          <p className="text-red-600 font-bold text-xs">Ø­Ø°Ù ÙƒØ§ÙØ© Ø§Ù„Ø£Ø¬Ù‡Ø²Ø© Ø§Ù„ÙˆÙ‡Ù…ÙŠØ© ØªÙ…Ø§Ù…Ø§Ù‹.</p>
        </div>
        <div className="flex items-center gap-3">
        <button
          onClick={async () => {
          ...
          className="px-8 py-4 bg-white text-red-700 rounded-2xl font-black text-xs shadow-lg border border-red-200"
        >
