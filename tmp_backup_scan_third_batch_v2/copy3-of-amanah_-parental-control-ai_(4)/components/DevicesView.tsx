...
const DevicesView: React.FC<DevicesViewProps> = ({ children, onToggleAppBlock, onUpdateDevice, lang }) => {
  ...
  return (
    <div className="space-y-10 pb-40 animate-in fade-in duration-700" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar flex-1 items-center">
           {children.map(c => (
             <button key={c.id} onClick={() => setSelectedChildId(c.id)} className={`flex items-center gap-4 px-6 py-3.5 rounded-full border-2 transition-all whitespace-nowrap ${selectedChildId === c.id ? 'bg-indigo-600 border-indigo-400 text-white shadow-xl' : 'bg-white border-slate-100 text-slate-500'}`}>
                {/* تم توحيد مقاس صورة الطفل هنا مع المقاس القياسي w-12 h-12 كحجم وسط للأزرار */}
                ...
                <div className="text-right">
                  ...
                  <span className={`text-[9px] font-black uppercase tracking-widest mt-1 block ${selectedChildId === c.id ? 'text-indigo-200' : 'text-slate-400'}`}>العمر: {c.age} {lang === 'ar' ? 'سنة' : 'Yrs'}</span>
                </div>
             </button>
           ))}
           <button 
             className="flex items-center gap-2 px-6 py-4 rounded-full border-2 border-dashed border-slate-300 text-slate-400 hover:border-indigo-400 hover:text-indigo-600 transition-all font-black text-xs shrink-0"
           >
             <ICONS.Plus />
             ربط جهاز جديد
           </button>
